...
@section('content')
    ...
    <div class="content">
        <a href="{{ route('snack.create') }}" class="btn btn-primary rounded-circle" id="fab">
                ...
                <div class="col-6">
                    <div class="card d-flex flex-column">
                        ...
                        <div class="card-body d-flex flex-column">
                            ...
                            <p class="card-text">Rp. {{ number_format($snack->price, 2, '.', '') }}</p>
                            <div class="button">

                                <!-- Tombol Status -->
                                <button id="statusBtn{{ $snack->id }}"
                                    class="btn btn-{{ $snack->status == 'ready' ? 'success' : 'danger' }}"
                                    onclick="updateStatus({{ $snack->id }})">
                                    @if ($snack->status == 'ready')
                                        <i class="fas fa-check"></i>
                                    @else
                                        <i class="fas fa-times"></i>
                                    @endif
                                </button>
                                <a href="{{ route('snack.edit', $snack->id) }}" class="btn btn-primary">Edit</a>
...
@push('script')
    <script>
        function updateStatus(snackId) {
            $.ajax({
                type: "POST",
                url: '{{ url('/update-snack-status/') }}' + '/' + snackId, // Perbaiki URL sesuai route
                data: {
                    _token: '{{ csrf_token() }}',
                    // id: snackId, // Ini tidak perlu karena `id` sudah ada di URL
                },
                success: function(data) {
                    // Perbarui tampilan tombol status
                    var statusBtn = $('#statusBtn' + snackId);

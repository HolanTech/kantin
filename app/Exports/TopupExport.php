...
class TopupExport implements FromQuery, WithHeadings, WithMapping, ShouldAutoSize, WithEvents
{
    use Exportable;

    protected $request;
    private $rowNumber = 0; // Tambahkan properti untuk nomor baris
    private $totalDebet = 0; // Menyimpan total debet
    ...
    {
        $query = Topup::query()->orderBy('tanggal', 'asc');
    ...
    public function map($topup): array
    {
        ...
        $this->totalDebet += $topup->debet; // Menambahkan debet ke total
        $this->totalAdmin += $topup->admin; // Menambahkan admin ke total
        return [
            $this->rowNumber,
            $topup->tanggal,
            $topup->user->rfid ?? 'N/A', // Pastikan relasi dengan user tersedia
            $topup->user->name ?? 'N/A', // Pastikan relasi dengan user tersedia
            $topup->payment,
            $topup->admin,
            $topup->debet,
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $event->sheet->setCellValue('G' . ($row + 1), $this->totalDebet);
            },
        ];
    }
}
